...
export const amountOneEnterAction = async () => {
  ...
  const myAmountOne = await prismadb.amount.findFirst({
    where: { 
      ...
      rank: "one" // le montant N°1 quel que soit la monnaie 
     }
  })
        ...
        { 
          ...
          await prismadb.activity.create({
            data: {
              ...
              action: "entrée dans une collecte snippet de : " + myAmountOne?.amount + myAmountOne?.currency + " de " + metrics.currentgroup + " participants."  // Json?
            }
          })
          // ##
