...
// AMOUNT THREE COLLECTION ENTER  
export const amountThreeEnterAction = async () => {
  ...
  {
    // 2- Verifier s'il est déjà dans une collecte ouverte ( isgroupcomplete = false ) de amountThree
        ...
        { 
          ...
          await prismadb.activity.create({
            data: {
              ...
              action: "entrée dans une collecte snippet de : " + myAmountThree?.amount + myAmountThree?.currency + " de " + metrics.currentgroup + " participants."  // Json?
            }
          })
          // ##
          ...
          // On diminue le crédit du connecté de amountThree
          await prismadb.profile.updateMany({  // en prod
            where: { 
              usercodepin: connectedProfile?.usercodepin,
            },
            data: { credit: connectedProfile?.credit - myAmountThree?.amount }
          })
        }
      }
      else{
        if( openCollectionCount === metrics?.currentNbrOfCollection )
        {
          ...
          if(metrics.currentNbrOfCollection === 1)
          {
            ...
            const theConcernedCollection = await prismadb.collection.findFirst({
              where: {
                ownId: openCollection?.ownId,
                amount: myAmountThree?.amount, // important 
                currency: myAmountThree?.currency, // important
          ...
          if(metrics.currentNbrOfCollection === 2)
          {
            ...
            if(topCollectionCount === downCollectionCount)
            {
              ...
              const decreaseCredit = await prismadb.profile.updateMany({  // en dev
                where: { 
                  usercodepin: connectedProfile?.usercodepin,
                },
                data: { credit: connectedProfile?.credit - myAmountThree?.amount }
              ...
              // On diminue le crédit du connecté de amountOne
              const decreaseCredit = await prismadb.profile.updateMany({  //
                where: { 
                  usercodepin: connectedProfile?.usercodepin,
                },
                data: { credit: connectedProfile?.credit - myAmountThree?.amount }
