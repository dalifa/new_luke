...
import { connectedAmountTwo, currentUserInfos } from "@/hooks/own-current-user";
...
export const amountTwoEnterAction = async () => {
  ...
  // 1- Vérifier s'il a du crédit et si son crédit est >= à amountTwo
      ...
      if( openCollectionCount !== metrics?.currentNbrOfCollection )
      {
        ...
        { 
          ...
          data: {
            ...
            action: "entrée dans une collecte snippet de : " + myAmountTwo?.amount + myAmountTwo?.currency + " de " + metrics.currentgroup + " participants."  // Json?
          }
          })
          // ##
