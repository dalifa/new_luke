  ...
  try 
  {
    ...
    */
    ...
    if( connected?.credit && concernedAmount && connected.credit >= concernedAmount?.amount )
    {
      ...
      // ##  
      // ############ LE NOOT EST > AU NBRE DE COLLECTE OUVERT #################
      if( metrics?.noot && metrics?.noot > openCollectionCount && openCollectionCount > 0 )
      {
        // ON EN CRÉE UN ET ON L'ENTRE COMME RANK = 1
        await prismadb.collectionList.create({
          data: {
      ...
      // ##
      // ############ LE NOOT EST === AU NBRE DE COLLECTE OUVERT ###############
      //############# OU LE NOOT EST < AU NBRE DE COLLECTE OUVERT ##############
      // noot = number of open totality 
      if( ( metrics?.noot && metrics?.noot === openCollectionCount && openCollectionCount > 0) || (metrics?.noot && metrics?.noot < openCollectionCount && openCollectionCount > 0) )
      {
              ...
              if(inTopFirstCount < 1)
              {
                ...
                if(inTopSecondCount < 1)
                {
                  ...
                  if(bottomCollection && inBottomCount < 1)
                  {
                    ...
                    await prismadb.collectionList.create({
                      data: {
                        ownId: metrics?.nois, // noit = next own id totality
                        amount: concernedAmount?.amount,
                        collectionType: ctype
                      }
                    })
