...
import { connectedAmountOne, connectedAmountTwo, currentUserInfos } from "@/hooks/own-current-user";
import { prismadb } from "@/lib/prismadb";
import { revalidatePath } from "next/cache";  
import { redirect } from "next/navigation"; 


// AMOUNT TWO TOTALITY COLLECTION ENTER    
export const amountTwoTotalityEnterAction = async () => {
  ...
  if( connectedProfile?.credit && myAmountTwo?.amount && connectedProfile.credit >= myAmountTwo.amount )
  {
    ...
    if( alreadyInCollectionCount < 1 ) 
    {
      ...
      // si le nbre trouvé n'est pas == à metrics.tocunooc
        ...
        { 
          ...
          await prismadb.activity.create({
            data: {
              ...
              action: "entrée dans une collecte totality de : " + myAmountTwo?.amount + myAmountTwo?.currency + " de " + metrics.tcg + " participants."  // Json?
            }
          })
          // ##
          ...
          const decreaseCredit = await prismadb.profile.updateMany({   // en prod
            ...
            data: { credit: connectedProfile?.credit - myAmountTwo?.amount }
          })
        }
      }
      else{
        if( openCollectionCount === metrics?.tocunooc )
        {
          ...
          await prismadb.activity.create({
            ...
            const theConcernedCollection = await prismadb.collection.findFirst({
              where: {
                ownId: openCollection?.ownId,
                amount: myAmountTwo?.amount, // important 
                currency: myAmountTwo?.currency, // important
