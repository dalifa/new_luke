// NE PLUS RENDRE PARTNER UN MEMBRE
"use client";
...
const formSchema = z.object({
  usercodepin: z.coerce.number().min(1, {
    message: "le user code pin"
  }),
});

const DontMakePartner = () => {
  const [isMounted, setIsMounted] = useState(false);
  const router = useRouter();
  const toastMessage =  'unactive partner successfully!';
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      //
      await axios.patch("/api/admin/dont-make-partner", values);
...
export default DontMakePartner