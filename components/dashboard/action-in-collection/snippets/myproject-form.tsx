...
export function MySnippetsProjectForm({ collectionId }: { collectionId: string }) {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <RefreshCw onClick={() => setIsOpen(true)} className='w-4 h-4 text-violet-700 cursor-pointer'/>
      </PopoverTrigger>
