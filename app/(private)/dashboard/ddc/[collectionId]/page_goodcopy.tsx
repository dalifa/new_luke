...
import { Ban, CircleUserRound, Gift, Handshake, Landmark, ToggleRight, UserRoundCheck } from 'lucide-react';
...
const Collection = async ({
  ...
  // on select la ddc en cours
  const currentDdc = await prismadb.collection.findFirst({
    where: { 
  ...
  const isOnlyDonatorCount = await prismadb.collectionParticipant.count({
    where: { 
      collectionId: currentDdc?.id, // id de la collecte
      isOnlyDonator: true
    }
  })
  //
  ...
  return (
    <div className='h-full flex items-center justify-center flex-col gap-5 bg-white'>
      <div className='w-4/5 lg:w-2/5 p-3 mb-5 border rounded text-center bg-white shadow-md shadow-blue-100'>
        ...
        {
          currentDdc?.groupStatus !== currentDdc?.group && (
            <div className='grid grid-cols-1 md:grid-cols-1 gap-y-2 font-medium'>
              <hr className='my-2'/>
              <p><span className='text-green-600'>{currentDdc?.groupStatus}</span> sur {currentDdc?.group}</p>
            </div>
          ) 
        }
      </div>
                ...
                <>
                {
                    ...
                    <>
                  {
                    myreceivers.map((myreceiver) => (
                      ...
                      /* le participant a déjà été mon donataire */
                      <Button className='bg-slate-500 hover:bg-slate-400 rounded-md text-white'>    
