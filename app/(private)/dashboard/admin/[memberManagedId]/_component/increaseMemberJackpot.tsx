'use client'
import { increaseMemberJackpot } from '@/actions/admin/increase_member_jackpot';
//
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useRef, useState } from 'react';
//
export function IncreaseMemberJackpot({ memberManagedId }: { memberManagedId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    //
    try {
      await increaseMemberJackpot(memberManagedId, formData);
      setSuccess(`Cagnotte augmenté de ${amountValue} `);
      setAmount("");
  ...
  return (
    <form 
      ...
      ref={formRef} 
      className="flex flex-col items-center justify-center gap-y-4 bg-emerald-600 rounded-md py-4"
    >
      <p className="text-white text-center text-xl font-medium">Member Jackpot</p>
