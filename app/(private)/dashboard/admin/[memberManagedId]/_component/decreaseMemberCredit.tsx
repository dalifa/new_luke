'use client'
import { decreaseMemberCredit } from '@/actions/admin/decrease_member_credit';
//
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useRef, useState } from 'react';
//
export function DecreaseMemberCredit({ memberManagedId }: { memberManagedId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // 
    try {
      await decreaseMemberCredit(memberManagedId, formData);
      setSuccess(`Crédit diminué de ${amountValue} `);
      setAmount("");
