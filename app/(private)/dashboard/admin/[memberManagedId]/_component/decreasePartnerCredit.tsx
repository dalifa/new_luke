'use client'
import { decreasePartnerCredit } from '@/actions/admin/decrease_partner_credit';

import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useRef, useState } from 'react';
//
export function DecreasePartnerCredit({ memberManagedId }: { memberManagedId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    //
    try {
      await decreasePartnerCredit(memberManagedId, formData);
      setSuccess(`Crédit partner diminué de ${amountValue} `);
      setAmount("");
  ...
  return (
    <form 
      ...
      ref={formRef} 
      className="flex flex-col items-center justify-center gap-y-4 bg-indigo-500 rounded-md py-4"
    >
      <p className="text-white text-center text-xl font-medium">Partner Credit</p>
