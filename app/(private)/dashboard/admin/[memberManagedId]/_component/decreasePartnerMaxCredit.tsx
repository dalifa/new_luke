'use client'
import { decreasePartnerMaxCredit } from '@/actions/admin/decrease_partner_max_credit';
//
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useRef, useState } from 'react';
//
export function DecreasePartnerMaxCredit({ memberManagedId }: { memberManagedId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    //
    try {
      await decreasePartnerMaxCredit(memberManagedId, formData);
      setSuccess(`Crédit partner max diminué de ${amountValue} `);
      setAmount("");
  ...
  return (
    <form 
      ...
      ref={formRef} 
      className="flex flex-col items-center justify-center gap-y-4 bg-cyan-600 rounded-md py-4"
    >
      <p className="text-white text-center text-xl font-medium">Partner Max Credit</p>
