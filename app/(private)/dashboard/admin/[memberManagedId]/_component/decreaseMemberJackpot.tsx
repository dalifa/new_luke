'use client'
import { decreaseMemberJackpot } from '@/actions/admin/decrease_member_jackpot';
//
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { useRef, useState } from 'react';
//
export function DecreaseMemberJackpot({ memberManagedId }: { memberManagedId: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    //
    try {
      await decreaseMemberJackpot(memberManagedId, formData);
      setSuccess(`Cagnotte diminué de ${amountValue} `);
      setAmount("");
