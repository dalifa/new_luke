...
import { donationAmountAction } from '@/actions/donationAmount';
    ...
    return (
        <div className='w-full flex items-center justify-center flex-col'>
            <div className='flex w-full p-4 md:w-3/5 items-center justify-center flex-col'>
                <Card className='flex flex-col w-full mt-0 md:mt-20 p-10 bg-white text-center items-center'>
                <p className='text-blue-600 text-lg md:text-xl font-semibold'>Confirmation de don</p>
                    <form action={donationToRecipientId} className='flex flex-col gap-y-5 pt-10'>
                        <div className='flex flex-col gap-1 text-center text-sm mb-5 text-slate-500'>
                        <p>Confirmez-vous le don libre de</p> 
                        {
                           profiles.map((profile) => (
                            <div key={profile.id} className='flex flex-col text-center gap-2'>
                                {
                                    profile.googleEmail === theParams?.email && (
                                        <>
                                            <p className='text-slate-500'>
                                            <span className='font-medium'>{theParams?.amount}{theParams?.currency} </span>
                                             à <span className='font-medium'>{profile.username}</span>
                                            </p>
                        ...
                        <p className='text-slate-500'>dans une collecte Snippet ?</p>
                        </div>
